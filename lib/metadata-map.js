...
Map.prototype.addFiles = function(files) {
  ...
  _.each(files, function(f) {
    var ext = path.extname(f);
    if(ext) {
      // TODO: Fix this because this won't work for Documents
      // since documents can have any file extension. They also
      // include folders
      var type = meta.getTypeFromExtension(ext);

      // as a backup, try to parse the path
      if(!type) {
        type = meta.getTypeFromPath(f);
      }
      ...
      if(type) {
        ...
        if(type.name === 'Document') {
          ...
          var rootIndex = _.indexOf(parts, 'documents');

          if(rootIndex != -1 && parts.length > rootIndex + 1) {

            var folderPath = _(parts)
              .take(rootIndex + 2)
              .value()
              .join('/');

            pushFile(type, folderPath);

            if(type.hasMetaXML) {
              pushFile(type, folderPath + '-meta.xml');
            }
          }
        }
      ...
      //
      //     if(type.hasMetaXML) {
      //       paths.push(folderPath + '-meta.xml');
      //     }
      //   }
      // }
    }
...
Map.prototype.getFilePathsForDeploy = function() {
  ...
  _.map(this.meta, function(metaList, metaType) {
    if(!metaList || !metaList.length) return;
    var type = meta.getTypeFromName(metaType);
...
Map.prototype.requiresMetadataDeploy = function() {
  ...
  _.forOwn(self.meta, function(value, key) {
    if(_.isArray(value) && value.length > 0) {
      var type = meta.getTypeFromName(key);
