...
function bootstrap() {
  return fs.ensureDir(paths.dir.home).then(function(){
    return fs.ensureDir(paths.dir.auth);
  }).then(function(){
    return fs.ensureDir(paths.dir.index);
  }).then(function(){
    return fs.exists(paths.file.globalConfig);
  }).then(function(configExists) {
    if(!configExists) {
      return fs.outputJson(paths.file.globalConfig, baseConfig);
    }
  }).catch(function(err) {
    // handle errors here
  })
...
function saveConfig() {
  return new Promise(function(resolve, reject) {
    ...
    fs.outputJson(paths.file.globalConfig, cfg).then(function(newCfg){
      cfg = newCfg;
      resolve(cfg);
    }).catch(function(err) {
      reject(err);
...
function getConfig() {
  return new Promise(function(resolve, reject) {
    ...
    fs.readJson(paths.file.globalConfig).then(function(newCfg) {
      cfg = newCfg;
      resolve(cfg);
    }).catch(function(err) {
      reject(err);
...
function hasCredential(org) {
  return fs.exists(paths.dir.auth + '/' + org + '.json')
}

function getCredential(org) {
  return fs.readJson(paths.dir.auth + '/' + org + '.json');
}

function saveCredential(org, data) {
  data.name = org;
  return fs.outputJson(paths.dir.auth + '/' + org + '.json', data);
}

function deleteCredential(org) {
  return hasCredential(org).then(function(doesExist) {
    if(doesExist) {
      return fs.unlink(paths.dir.auth + '/' + org + '.json');
    }
  })
}
...
function listCredentials() {
  return fs.readdir(authDir).then(function(orgs) {
    ...
    var promises = _(orgs)
      .map(function(o) {
        if(/\.json/.test(c)) {
          return rs.readJson(paths.dir.auth + '/' + c).then(function(org) {
            if(!org.name) {
